...
export default function LoginCard() {
  ...
  const handleOnBlur = (id: string, ref: React.RefObject<HTMLInputElement | null>) => {
    if (ref.current?.value !== null && ref.current?.value !== "") {
      (document.getElementById(id) as HTMLDivElement).style.opacity = "0";
    }
  }

  const handleOnFocus = (id: string, ref: React.RefObject<HTMLInputElement | null>) => {
  ...
  const handleLogin = async () => {
    const checkUser = await user({ username: userRef.current?.value || "", password: passRef.current?.value || "" });
